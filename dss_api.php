...
// Maintance Month list
function maintance_mlist()
{
class maintance_mlist {
      public $status = "";
      public $msg  = "";
      public $maintance_mlist  = "";  
     
   }
   $maintance_mlist=new maintance_mlist();

	global $conn;
	$sql="SELECT * FROM sm_maintance where sm_m_status='1'";
$result=$conn->query($sql);
...
$dss_data[] = array(  
             'sm_m_id'               =>     $row["sm_m_id"],  
             'sm_m_y'          =>     $row["sm_m_y"],  
             'sm_m_amount'     =>     $row["sm_m_amount"],
             'sm_m_month'   =>     $row["sm_m_month"], 
             'sm_m_year'          =>     $row["sm_m_year"],  
             'sm_m_status'          =>     $row["sm_m_status"]

           );  
	}
	$maintance_mlist->status="1";
$maintance_mlist->msg="Maintance Month List Fetch Successfully";
$maintance_mlist->maintance_mlist=$dss_data;

}
else
{
$maintance_mlist->status="0";
$maintance_mlist->msg="No Maintance Month List  found";
$maintance_mlist->maintance_mlist=array();

}
header("Access-Control-Allow-Origin: *");
header("Content-Type: application/json; charset=UTF-8");
header("HTTP/1.1 201 OK");
echo json_encode($maintance_mlist);
}
